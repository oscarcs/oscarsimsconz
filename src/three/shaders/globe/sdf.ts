// CPU-side earth SDF texture generation.
// Decodes delta-encoded 8-bit vectors into line segments,
// computes signed distance field with even-odd winding rule.
...
const VECT_COUNT = 3040;
const SHAPE_COUNT = 101;
const MAX_EXTENT = 756.0;
const ASPECT_X = 2.0; // Horizontal aspect stretch for equirectangular

// Vector data: 4 bytes per int, each byte is an 8-bit vector
// with two signed 4-bit (X low nibble, Y high nibble) components.
// prettier-ignore
const vects = [
...
// Shape data: bit layout [31:20]=vertex_index [19:10]=y_pos [9:0]=x_pos
// prettier-ignore
const shapes = [
...
// Decode a signed 4-bit value from a nibble
function signed4(v: number): number {
    v = v & 0x0f;
    return v >= 8 ? v - 16 : v;
}

// Pre-compute all line segments from the vector/shape data
function buildSegments() {
    // Find shape index for each vector
    const shapeStarts = new Int32Array(SHAPE_COUNT);
    for (let j = 0; j < SHAPE_COUNT; j++) {
        shapeStarts[j] = (shapes[j] >>> 20) & 0xfff;
    }

    // Shape origins
    const originX = new Float64Array(SHAPE_COUNT);
    const originY = new Float64Array(SHAPE_COUNT);
    for (let j = 0; j < SHAPE_COUNT; j++) {
        originX[j] = (shapes[j] & 0x3ff) / MAX_EXTENT;
        originY[j] = ((shapes[j] >>> 10) & 0x3ff) / MAX_EXTENT;
    }

    // Flat arrays for segment endpoints
    const x0 = new Float64Array(VECT_COUNT);
    const y0 = new Float64Array(VECT_COUNT);
    const x1 = new Float64Array(VECT_COUNT);
    const y1 = new Float64Array(VECT_COUNT);

    let shapeIdx = 0;
    let px = originX[0];
    let py = originY[0];

    for (let i = 0; i < VECT_COUNT; i++) {
        // Check if we've entered a new shape (use while in case multiple shapes share a start)
        while (shapeIdx + 1 < SHAPE_COUNT && i >= shapeStarts[shapeIdx + 1]) {
            shapeIdx++;
        }
        // Reset pen to shape origin on first vector or shape change
        if (i === 0 || (i === shapeStarts[shapeIdx])) {
            px = originX[shapeIdx];
            py = originY[shapeIdx];
        }

        // Extract byte from packed int
        const word = vects[i >> 2];
        const byte = (word >>> ((i & 3) * 8)) & 0xff;
        const dx = signed4(byte) / MAX_EXTENT;
        const dy = signed4(byte >>> 4) / MAX_EXTENT;
...
// Convert a 32-bit float to a 16-bit half-float stored in a uint16.
...
export function generateEarthSDF(width: number, height: number): DataTexture {
    const { x0, y0, x1, y1 } = buildSegments();
    ...
    for (let row = 0; row < height; row++) {
        ...
        for (let col = 0; col < width; col++) {
            ...
            for (let i = 0; i < VECT_COUNT; i++) {
                ...
                const frac = Math.max(
                    0,
                    Math.min(1, ((px - x0[i]) * lx + (py - y0[i]) * ly) / lenSq),
                );
                let nearX = x0[i] + lx * frac - px;
                const nearY = y0[i] + ly * frac - py;

                // Tile horizontally
                nearX = ((nearX % 1.0) + 1.5) % 1.0 - 0.5;

                const dist = Math.sqrt(
                    (nearX * ASPECT_X) * (nearX * ASPECT_X) + nearY * nearY,
                );
                sdf =
                    Math.min(Math.abs(sdf), dist) * (sdf >= 0 ? 1 : -1);

                // Even-odd winding rule for sign
                const minY = y0[i] < y1[i] ? y0[i] : y1[i];
                const maxY = y0[i] > y1[i] ? y0[i] : y1[i];
                if (py >= minY && py < maxY) {
                    const t = (py - y0[i]) / (y1[i] - y0[i]);
                    const xIntercept = x0[i] + (x1[i] - x0[i]) * t;
                    if (px > xIntercept) {
                        sdf = -sdf;
                    }
                }
